...
test('document is dirty if text changes', () => {
    let doc = Document.fromStorageData(UnitTestFrontMatterType, { uniqueId: 'a', ns: 'db', name: 'title', frontMatter: { foo: [1, 2, 3] }, text: "hello" })
    T.eq(doc.frontMatter, { foo: [1, 2, 3] })
    T.eq(doc.text, "hello")
    T.eq(doc.isDirty, false)
    doc.text = "hello!"
    T.eq(doc.isDirty, true as any)
    T.eq(doc.getDocumentStorageData(), { uniqueId: 'a', ns: 'db', name: 'title', frontMatter: { foo: [1, 2, 3] }, text: "hello!" })
})
...
test('document is dirty if front matter changes', () => {
    ...
    T.eq(doc.isDirty, true as any)
    T.eq(doc.getDocumentStorageData(), { uniqueId: 'a', ns: 'db', name: 'title', frontMatter: { foo: [1, 10, 3] }, text: "hello" })
})
